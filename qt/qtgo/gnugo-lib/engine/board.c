 ...
 */
#if 1
#define PARANOID1(internal_state, x, pos) ASSERT1(internal_state, x, pos)
#else
...
#define LIBERTY(internal_state, pos) \
  (internal_state->board[pos] == EMPTY)

#define UNMARKED_LIBERTY(internal_state, pos) \
  (internal_state->board[pos] == EMPTY && ml[pos] != liberty_mark)
 ...
 */
#define UNMARKED_COLOR_STRING(internal_state, pos, color)\
  (internal_state->board[pos] == color\
   && string[string_number[pos]].mark != string_mark)

#define MARKED_COLOR_STRING(internal_state, pos, color)\
  (internal_state->board[pos] == color\
   && string[string_number[pos]].mark == string_mark)

#define MARK_STRING(pos) string[string_number[pos]].mark = string_mark

#define STRING_AT_VERTEX(internal_state, pos, s, color)\
  ((internal_state->board[pos] == color) && string_number[pos] == (s))

#define NEIGHBOR_OF_STRING(internal_state, pos, s, color)\
  (STRING_AT_VERTEX(internal_state, SOUTH(pos), s, color)\
   || STRING_AT_VERTEX(internal_state, WEST(pos), s, color)\
   || STRING_AT_VERTEX(internal_state, NORTH(pos), s, color)\
   || STRING_AT_VERTEX(internal_state, EAST(pos), s, color))
 ...
 * the specified one".
 */
#define NON_SOUTH_NEIGHBOR_OF_STRING(internal_state, pos, s, color)\
  (STRING_AT_VERTEX(internal_state, SOUTH(pos), s, color)\
   || STRING_AT_VERTEX(internal_state, WEST(pos), s, color)\
   || STRING_AT_VERTEX(internal_state, EAST(pos), s, color))
  
#define NON_WEST_NEIGHBOR_OF_STRING(internal_state, pos, s, color)\
  (STRING_AT_VERTEX(internal_state, WEST(pos), s, color)\
   || STRING_AT_VERTEX(internal_state, NORTH(pos), s, color)\
   || STRING_AT_VERTEX(internal_state, SOUTH(pos), s, color))
  
#define NON_NORTH_NEIGHBOR_OF_STRING(internal_state, pos, s, color)\
  (STRING_AT_VERTEX(internal_state, NORTH(pos), s, color)\
   || STRING_AT_VERTEX(internal_state, EAST(pos), s, color)\
   || STRING_AT_VERTEX(internal_state, WEST(pos), s, color))
  
#define NON_EAST_NEIGHBOR_OF_STRING(internal_state, pos, s, color)\
  (STRING_AT_VERTEX(internal_state, EAST(pos), s, color)\
   || STRING_AT_VERTEX(internal_state, SOUTH(pos), s, color)\
   || STRING_AT_VERTEX(internal_state, NORTH(pos), s, color))
...
#define DO_ADD_STONE(internal_state, pos, color)\
  do {\
...
static int do_accuratelib(struct board_lib_state_struct *internal_state, int pos, int color, int maxlib, int *libs);

static int is_superko_violation(struct board_lib_state_struct *internal_state, int pos, int color, enum ko_rules type);

static void new_position(struct board_lib_state_struct *internal_state);
static int propagate_string(struct board_lib_state_struct *internal_state, int stone, int str);
static void find_liberties_and_neighbors(struct board_lib_state_struct *internal_state, int s);
static int do_remove_string(struct board_lib_state_struct *internal_state, int s);
static void do_commit_suicide(struct board_lib_state_struct *internal_state, int pos, int color);
static void do_play_move(struct board_lib_state_struct *internal_state, int pos, int color);
...
void
store_board(struct board_lib_state_struct* internal_state, struct board_state *state)
{
  int k;

  gg_assert(internal_state, internal_state->stackp == 0);

  state->board_size = internal_state->board_size;

  memcpy(state->board, internal_state->board, sizeof(internal_state->board));
  memcpy(state->initial_board, internal_state->initial_board, sizeof(internal_state->initial_board));

  state->board_ko_pos = internal_state->board_ko_pos;
  state->white_captured = internal_state->white_captured;
  state->black_captured = internal_state->black_captured;

  state->initial_board_ko_pos = internal_state->initial_board_ko_pos;
  state->initial_white_captured = internal_state->initial_white_captured;
  state->initial_black_captured = internal_state->initial_black_captured;
  
  state->move_history_pointer = internal_state->move_history_pointer;
  for (k = 0; k < internal_state->move_history_pointer; k++) {
    state->move_history_color[k] = internal_state->move_history_color[k];
    state->move_history_pos[k] = internal_state->move_history_pos[k];
    state->move_history_hash[k] = internal_state->move_history_hash[k];
  }

  state->komi = internal_state->komi;
  state->handicap = internal_state->handicap;
  state->move_number = internal_state->movenum;
}
...
void
restore_board(struct board_lib_state_struct* internal_state, struct board_state *state)
{
  int k;

  gg_assert(internal_state, internal_state->stackp == 0);

  internal_state->board_size = state->board_size;

  memcpy(internal_state->board, state->board, sizeof(internal_state->board));
  memcpy(internal_state->initial_board, state->initial_board, sizeof(internal_state->initial_board));

  internal_state->board_ko_pos = state->board_ko_pos;
  internal_state->white_captured = state->white_captured;
  internal_state->black_captured = state->black_captured;

  internal_state->initial_board_ko_pos = state->initial_board_ko_pos;
  internal_state->initial_white_captured = state->initial_white_captured;
  internal_state->initial_black_captured = state->initial_black_captured;
  
  internal_state->move_history_pointer = state->move_history_pointer;
  for (k = 0; k < internal_state->move_history_pointer; k++) {
    internal_state->move_history_color[k] = state->move_history_color[k];
    internal_state->move_history_pos[k] = state->move_history_pos[k];
    internal_state->move_history_hash[k] = state->move_history_hash[k];
  }

  internal_state->komi = state->komi;
  internal_state->handicap = state->handicap;
  internal_state->movenum = state->move_number;
...
void
clear_board(struct board_lib_state_struct *internal_state)
{
  int k;

  gg_assert(internal_state, internal_state->board_size > 0 && internal_state->board_size <= MAX_BOARD);
  
  memset(internal_state->board, EMPTY, sizeof(internal_state->board));
  memset(internal_state->initial_board, EMPTY, sizeof(internal_state->initial_board));
  for (k = 0; k < BOARDSIZE; k++) {
    if (!ON_BOARD2(internal_state, I(k), J(k))) {
      internal_state->board[k] = GRAY;
      internal_state->initial_board[k] = GRAY;
    }
  }

  internal_state->board_ko_pos = NO_MOVE;
  internal_state->white_captured = 0;
  internal_state->black_captured = 0;

  komaster = EMPTY;
  kom_pos = NO_MOVE;

  internal_state->initial_board_ko_pos = NO_MOVE;
  internal_state->initial_white_captured = 0;
  internal_state->initial_black_captured = 0;

  internal_state->move_history_pointer = 0;
  internal_state->movenum = 0;
...
int
test_gray_border(struct board_lib_state_struct *internal_state)
{
  int k;
  ...
  for (k = 0; k < BOARDSIZE; k++)
    if (!ON_BOARD2(internal_state, I(k), J(k)))
      if (internal_state->board[k] != GRAY)
      	return k;
...
trymove(struct board_lib_state_struct *internal_state, int pos, int color, const char *message, int str)
{
  ...
  if (!do_trymove(internal_state, pos, color, 0))
    return 0;
...
tryko(struct board_lib_state_struct *internal_state, int pos, int color, const char *message)
{
  ...
  if (internal_state->sgf_dumptree) {
     ...
     * with this, though.
     */
    sgftreeAddPlayLast(internal_state->sgf_dumptree, color, -1, -1);
    sgftreeAddComment(internal_state->sgf_dumptree, "tenuki (ko threat)");
    sgftreeAddPlayLast(internal_state->sgf_dumptree, OTHER_COLOR(color), -1, -1);
    sgftreeAddComment(internal_state->sgf_dumptree, "tenuki (answers ko threat)");
...
really_do_trymove(struct board_lib_state_struct* internal_state, int pos, int color)
{
  ...
  if (pos != PASS_MOVE) {
    PUSH_VALUE(internal_state->black_captured);
    PUSH_VALUE(internal_state->white_captured);
    do_play_move(internal_state, pos, color);
  }
}
...
do_trymove(struct board_lib_state_struct *internal_state, int pos, int color, int ignore_ko)
{
  ...
  if (pos != PASS_MOVE) {
    ...
    if (!ignore_ko && pos == internal_state->board_ko_pos) {
      if (internal_state->board[WEST(pos)] == OTHER_COLOR(color)
      || internal_state->board[EAST(pos)] == OTHER_COLOR(color)) {
	return 0;
      }
    }

    /* 5. Test for suicide. */
    if (is_suicide(internal_state, pos, color))
      return 0;
  }
  
  /* Check for stack overflow. */
  if (internal_state->stackp >= MAXSTACK-2) {
    fprintf(stderr, 
	    "gnugo: Truncating search. This is beyond my reading ability!\n");
    /* FIXME: Perhaps it's best to just assert here and be done with it? */
    if (0) {
      ASSERT1(internal_state, 0 && "trymove stack overflow", pos);
    }
#if 0
    if (verbose > 0) {
      showboard(0);
...
void
popgo(struct board_lib_state_struct *internal_state)
{
  ...
  if (internal_state->sgf_dumptree) {
     ...
     */
    if (sgfGetCharProperty(internal_state->sgf_dumptree->lastnode, "C ", &sgf_comment)
	&& strncmp(sgf_comment, "tryko:", 6) == 0)
      is_tryko = 1;
    
    gg_snprintf(buf, 100, "(next variation: %d)", internal_state->count_variations);
    sgftreeAddComment(internal_state->sgf_dumptree, buf);
    internal_state->sgf_dumptree->lastnode = internal_state->sgf_dumptree->lastnode->parent;

    /* After tryko() we need to undo two pass nodes too. */
    if (is_tryko)
      internal_state->sgf_dumptree->lastnode = internal_state->sgf_dumptree->lastnode->parent->parent;
  }
}
 ...
 * since the last BEGIN_CHANGE_RECORD(). Also internal_state->stackp is decreased and
...
undo_trymove(struct board_lib_state_struct *internal_state)
{
  ...
  if (0) {
    gprintf(internal_state, "Change stack size = %d\n", change_stack_pointer - change_stack);
    gprintf(internal_state, "Vertex stack size = %d\n", vertex_stack_pointer - vertex_stack);
  }
...
void
dump_stack(struct board_lib_state_struct *internal_state)
{
...
do_dump_stack(struct board_lib_state_struct *internal_state)
{
  ...
  for (n = 0; n < internal_state->stackp; n++)
    gprintf(internal_state, "%o%s:%1m ", move_color[n] == BLACK ? "B" : "W", stack[n]);
}
...
static void
reset_move_history(struct board_lib_state_struct *internal_state)
{
  memcpy(internal_state->initial_board, internal_state->board, sizeof(internal_state->board));
  internal_state->initial_board_ko_pos = internal_state->board_ko_pos;
  internal_state->initial_white_captured = internal_state->white_captured;
  internal_state->initial_black_captured = internal_state->black_captured;
  internal_state->move_history_pointer = 0;
}
 ...
 */
static void
play_move_no_history(struct board_lib_state_struct *internal_state, int pos, int color, int update_internals)
{
#if CHECK_HASHING
  Hash_data oldkey;
  ...
  if (pos != PASS_MOVE) {
    ...
    if (!is_suicide(internal_state, pos, color))
      ...
      do_commit_suicide(internal_state, pos, color);
...
static void
replay_move_history(struct board_lib_state_struct *internal_state, int n)
{
  int k;
  
  memcpy(internal_state->board, internal_state->initial_board, sizeof(internal_state->board));
  internal_state->board_ko_pos = internal_state->initial_board_ko_pos;
  internal_state->white_captured = internal_state->initial_white_captured;
  internal_state->black_captured = internal_state->initial_black_captured;
  new_position(internal_state);

  for (k = 0; k < n; k++)
    play_move_no_history(internal_state, internal_state->move_history_pos[k], internal_state->move_history_color[k], 0);
...
play_move(struct board_lib_state_struct *internal_state, int pos, int color)
{
    //printf("play_move\n");
  ASSERT1(internal_state, internal_state->stackp == 0, pos);
  ASSERT1(internal_state, color == WHITE || color == BLACK, pos);
  ASSERT1(internal_state, pos == PASS_MOVE || ON_BOARD1(internal_state, pos), pos);
  ASSERT1(internal_state, pos == PASS_MOVE || internal_state->board[pos] == EMPTY, pos);
  ASSERT1(internal_state, komaster == EMPTY && kom_pos == NO_MOVE, pos);

  if (internal_state->move_history_pointer >= MAX_MOVE_HISTORY) {
    ...
    int saved_board_ko_pos = internal_state->board_ko_pos;
    int saved_white_captured = internal_state->white_captured;
    int saved_black_captured = internal_state->black_captured;
    memcpy(saved_board, internal_state->board, sizeof(internal_state->board));

    replay_move_history(internal_state, number_collapsed_moves);
    ...
    for (k = number_collapsed_moves; k < internal_state->move_history_pointer; k++) {
      internal_state->move_history_color[k - number_collapsed_moves] = internal_state->move_history_color[k];
      internal_state->move_history_pos[k - number_collapsed_moves] = internal_state->move_history_pos[k];
      internal_state->move_history_hash[k - number_collapsed_moves] = internal_state->move_history_hash[k];
    }
    internal_state->move_history_pointer -= number_collapsed_moves;

    memcpy(internal_state->board, saved_board, sizeof(internal_state->board));
    internal_state->board_ko_pos = saved_board_ko_pos;
    internal_state->white_captured = saved_white_captured;
    internal_state->black_captured = saved_black_captured;
    new_position(internal_state);
  }
  ...
  play_move_no_history(internal_state, pos, color, 1);
 ...
 */
int
undo_move(struct board_lib_state_struct *internal_state, int n)
{
  gg_assert(internal_state, internal_state->stackp == 0);
  
  /* Fail if and only if the move history is too short. */
  if (internal_state->move_history_pointer < n)
    return 0;

  replay_move_history(internal_state, internal_state->move_history_pointer - n);
  internal_state->move_history_pointer -= n;
  internal_state->movenum -= n;
 ...
 */
int
get_last_opponent_move(struct board_lib_state_struct *internal_state, int color)
{
  int k;
  
  for (k = internal_state->move_history_pointer - 1; k >= 0; k--)
    if (internal_state->move_history_color[k] == OTHER_COLOR(color))
      return internal_state->move_history_pos[k];
 ...
 */
int
get_last_move(struct board_lib_state_struct *internal_state)
{
  if (internal_state->move_history_pointer == 0)
    return PASS_MOVE;

  return internal_state->move_history_pos[internal_state->move_history_pointer - 1];
}
 ...
 */
int
get_last_player(struct board_lib_state_struct *internal_state)
{
  if (internal_state->move_history_pointer == 0)
    return EMPTY;

  return internal_state->move_history_color[internal_state->move_history_pointer - 1];
}
...
int 
is_legal(struct board_lib_state_struct *internal_state, int pos, int color)
{
  ...
  if (pos == internal_state->board_ko_pos) {
  ...
  if (internal_state->stackp >= MAXSTACK-2) {
    ...
    /* FIXME: Perhaps it's best to just assert here and be done with it? */
    if (0) {
      ASSERT1(internal_state, 0 && "is_legal stack overflow", pos);
    }
    return 0;
  }
 ...
 */
int 
is_suicide(struct board_lib_state_struct *internal_state, int pos, int color)
{
  ASSERT_ON_BOARD1(internal_state, pos);
  ASSERT1(internal_state, internal_state->board[pos] == EMPTY, pos);

  /* Check for suicide. */
  if (LIBERTY(internal_state, SOUTH(pos))
      || (ON_BOARD(internal_state, SOUTH(pos))
      && ((internal_state->board[SOUTH(pos)] == color) ^ (LIBERTIES(SOUTH(pos)) == 1))))
    return 0;

  if (LIBERTY(internal_state, WEST(pos))
      || (ON_BOARD(internal_state, WEST(pos))
      && ((internal_state->board[WEST(pos)] == color) ^ (LIBERTIES(WEST(pos)) == 1))))
    return 0;

  if (LIBERTY(internal_state, NORTH(pos))
      || (ON_BOARD(internal_state, NORTH(pos))
      && ((internal_state->board[NORTH(pos)] == color) ^ (LIBERTIES(NORTH(pos)) == 1))))
    return 0;

  if (LIBERTY(internal_state, EAST(pos))
      || (ON_BOARD(internal_state, EAST(pos))
      && ((internal_state->board[EAST(pos)] == color) ^ (LIBERTIES(EAST(pos)) == 1))))
    return 0;
 ...
 */
int 
is_illegal_ko_capture(struct board_lib_state_struct *internal_state, int pos, int color)
{
  ASSERT_ON_BOARD1(internal_state, pos);
  ASSERT1(internal_state, internal_state->board[pos] == EMPTY, pos);

  return (pos == internal_state->board_ko_pos
      && ((internal_state->board[WEST(pos)] == OTHER_COLOR(color))
          || (internal_state->board[EAST(pos)] == OTHER_COLOR(color))));
}
 ...
 *
 * This function is only valid when internal_state->stackp == 0 since there is no
 * tracking of superko for trymoves.
 */
int
is_allowed_move(struct board_lib_state_struct *internal_state, int pos, int color)
{
  gg_assert(internal_state, internal_state->stackp == 0);
   ...
   *    suffices.
   */
  if (internal_state->ko_rule != NONE
      && pos == internal_state->board_ko_pos
      && (internal_state->board[WEST(pos)] == OTHER_COLOR(color)
      || internal_state->board[EAST(pos)] == OTHER_COLOR(color)))
    return 0;
   ...
   */
  if (is_suicide(internal_state, pos, color))
    if (internal_state->suicide_rule == FORBIDDEN
    || (internal_state->suicide_rule == ALLOWED
        && !has_neighbor(internal_state, pos, color)))
      return 0;
   ...
   */
  if (is_superko_violation(internal_state, pos, color, internal_state->ko_rule))
    return 0;
		 ...
		 int *is_conditional_ko, int consider_conditional_ko)
{
  ...
  int ko_move;
  int kpos;
  int previous_board_ko_pos = internal_state->board_ko_pos;

  *is_conditional_ko = 0;
  ko_move = is_ko(internal_state, pos, color, &kpos);
  ...
  if (ko_move) {
     ...
     */
    if (komaster == WEAK_KO) {
      if (pos != internal_state->board_ko_pos && !DIAGONAL_NEIGHBORS(kpos, kom_pos))
	return 0;
    }
  }
  ...
  if (!trymove(internal_state, pos, color, message, str)) {
    ...
    if (!tryko(internal_state, pos, color, message))
      return 0; /* Suicide. */
...
is_edge_vertex(struct board_lib_state_struct *internal_state, int pos)
{
  ...
  if (!ON_BOARD(internal_state, SW(pos))
      || !ON_BOARD(internal_state, NE(pos)))
    return 1;
...
int
edge_distance(struct board_lib_state_struct *internal_state, int pos)
{
  int i = I(pos);
  int j = J(pos);
  ASSERT_ON_BOARD1(internal_state, pos);
  return gg_min(gg_min(i, internal_state->board_size-1 - i), gg_min(j, internal_state->board_size-1 - j));
}
...
is_corner_vertex(struct board_lib_state_struct *internal_state, int pos)
{
  ...
  if ((!ON_BOARD(internal_state, WEST(pos)) || !ON_BOARD(internal_state, EAST(pos)))
      && (!ON_BOARD(internal_state, SOUTH(pos)) || !ON_BOARD(internal_state, NORTH(pos))))
    return 1;
 ...
 */
int
rotate1(struct board_lib_state_struct *internal_state, int pos, int rot)
{
  int bs = internal_state->board_size - 1;
  int i = I(pos);
  int j = J(pos);
  gg_assert(internal_state, rot >= 0 && rot < 8);
 ...
 */
int
are_neighbors(struct board_lib_state_struct *internal_state, int pos1, int pos2)
{
  if (internal_state->board[pos1] == EMPTY) {
    if (internal_state->board[pos2] == EMPTY)
      return (gg_abs(pos1 - pos2) == NS || gg_abs(pos1 - pos2) == WE);
    else
      return neighbor_of_string(internal_state, pos1, pos2);
  }
  else {
    if (internal_state->board[pos2] == EMPTY)
      return neighbor_of_string(internal_state, pos2, pos1);
    else
      return adjacent_strings(internal_state, pos1, pos2);
  }
}
...
findlib(struct board_lib_state_struct *internal_state, int str, int maxlib, int *libs)
{
  ...
  if (liberties <= MAX_LIBERTIES || maxlib <= MAX_LIBERTIES) {
    ...
    for (k = 0, pos = FIRST_STONE(s);
	 ...
	 pos = NEXT_STONE(pos)) {
      if (UNMARKED_LIBERTY(internal_state, SOUTH(pos))) {
	libs[k++] = SOUTH(pos);
	...
	if (k >= maxlib)
      ...
      if (UNMARKED_LIBERTY(internal_state, WEST(pos))) {
	libs[k++] = WEST(pos);
	...
	if (k >= maxlib)
      ...
      if (UNMARKED_LIBERTY(internal_state, EAST(pos))) {
	libs[k++] = EAST(pos);
...
int
fastlib(struct board_lib_state_struct *internal_state, int pos, int color, int ignore_captures)
{
  int ally1 = -1;
  int ally2 = -1;
   ...
   */
  if (internal_state->board[SOUTH(pos)] == color) {
    ally1 = string_number[SOUTH(pos)];

    if (internal_state->board[WEST(pos)] == color
	&& string_number[WEST(pos)] != ally1) {
      ally2 = string_number[WEST(pos)];

      if (internal_state->board[NORTH(pos)] == color
	  && string_number[NORTH(pos)] != ally1
	  && string_number[NORTH(pos)] != ally2)
	return -1;
    }
    else if (internal_state->board[NORTH(pos)] == color
	     && string_number[NORTH(pos)] != ally1)
      ally2 = string_number[NORTH(pos)];

    if (internal_state->board[EAST(pos)] == color
  ...
  else if (internal_state->board[WEST(pos)] == color) {
    ...
    else if (internal_state->board[EAST(pos)] == color
	     && string_number[EAST(pos)] != ally1)
      ally2 = string_number[EAST(pos)];
  }
  else if (internal_state->board[NORTH(pos)] == color) {
  ...
  if (ignore_captures) {
	...
	fast_liberties++;
      ...
      if (LIBERTY(internal_state, SOUTH(pos))
      && !NON_SOUTH_NEIGHBOR_OF_STRING(internal_state, SOUTH(pos), ally1, color))
	fast_liberties++;
      if (LIBERTY(internal_state, WEST(pos))
      && !NON_WEST_NEIGHBOR_OF_STRING(internal_state, WEST(pos), ally1, color))
	fast_liberties++;
      if (LIBERTY(internal_state, NORTH(pos))
      && !NON_NORTH_NEIGHBOR_OF_STRING(internal_state, NORTH(pos), ally1, color))
	fast_liberties++;
      if (LIBERTY(internal_state, EAST(pos))
      && !NON_EAST_NEIGHBOR_OF_STRING(internal_state, EAST(pos), ally1, color))
	fast_liberties++;
      ...
      if (LIBERTY(internal_state, SOUTH(pos))
      && !NON_SOUTH_NEIGHBOR_OF_STRING(internal_state, SOUTH(pos), ally1, color)
      && !NON_SOUTH_NEIGHBOR_OF_STRING(internal_state, SOUTH(pos), ally2, color))
	fast_liberties++;
      if (LIBERTY(internal_state, WEST(pos))
      && !NON_WEST_NEIGHBOR_OF_STRING(internal_state, WEST(pos), ally1, color)
      && !NON_WEST_NEIGHBOR_OF_STRING(internal_state, WEST(pos), ally2, color))
	fast_liberties++;
      if (LIBERTY(internal_state, NORTH(pos))
      && !NON_NORTH_NEIGHBOR_OF_STRING(internal_state, NORTH(pos), ally1, color)
      && !NON_NORTH_NEIGHBOR_OF_STRING(internal_state, NORTH(pos), ally2, color))
	fast_liberties++;
      if (LIBERTY(internal_state, EAST(pos))
      && !NON_EAST_NEIGHBOR_OF_STRING(internal_state, EAST(pos), ally1, color)
      && !NON_EAST_NEIGHBOR_OF_STRING(internal_state, EAST(pos), ally2, color))
	fast_liberties++;

      fast_liberties += string[ally1].liberties + string[ally2].liberties
    - count_common_libs(internal_state, string[ally1].origin, string[ally2].origin) - 1;
    }
  }
   ...
   */
    ...
    for (k = 0; k < 4; k++) {
      ...
      if (LIBERTY(internal_state, neighbor)
      && (ally1 < 0 || !NEIGHBOR_OF_STRING(internal_state, neighbor, ally1, color))
      && (ally2 < 0 || !NEIGHBOR_OF_STRING(internal_state, neighbor, ally2, color)))
	fast_liberties++;
      else if (internal_state->board[neighbor] == OTHER_COLOR(color)	/* A capture */
	...
	fast_liberties += string[ally2].liberties
      - count_common_libs(internal_state, string[ally1].origin, string[ally2].origin);
    }
  }
 ...
 */
int
approxlib(struct board_lib_state_struct *internal_state, int pos, int color, int maxlib, int *libs)
{
  int liberties;
  ...
  if (!libs) {
    ...
    liberties = fastlib(internal_state, pos, color, 1);
    if (liberties >= 0) {
...
static int
do_approxlib(struct board_lib_state_struct *internal_state, int pos, int color, int maxlib, int *libs)
{
  int k;
  int liberties = 0;
 ...
 */
static int
slow_approxlib(struct board_lib_state_struct *internal_state, int pos, int color, int maxlib, int *libs)
{
  int k;
  int liberties = 0;
  ...
  for (k = 0; k < 4; k++) {
    int d = delta[k];
    if (UNMARKED_LIBERTY(internal_state, pos + d)) {
      if (libs)
    ...
    else if (internal_state->board[pos + d] == color
	...
	for (l = 0; l < 4; l++) {
	  int d2 = delta[l];
      if (UNMARKED_LIBERTY(internal_state, pos2 + d2)) {
	    if (libs)
 ...
 */
int
accuratelib(struct board_lib_state_struct *internal_state, int pos, int color, int maxlib, int *libs)
{
  int liberties;
  ...
  if (!libs) {
    ...
    liberties = fastlib(internal_state, pos, color, 0);
    if (liberties >= 0) {
      /* Since fastlib() always returns precise result and doesn't take
       * `maxlib' into account, we set threshold to MAXLIBS so that this
       * result is used regardless of any `maxlib' passed.
...
static int
do_accuratelib(struct board_lib_state_struct *internal_state, int pos, int color, int maxlib, int *libs)
{
  int k, l;
  int liberties = 0;
  int lib;
  ...
  for (k = 0; k < 4; k++) {
    ...
    if (UNMARKED_LIBERTY(internal_state, pos2)) {
      /* A trivial liberty */
      if (libs)
	libs[liberties] = pos2;
      liberties++;
    ...
    else if (UNMARKED_COLOR_STRING(internal_state, pos2, color)) {
	...
	int stone = pos2;
	do {
      if (UNMARKED_LIBERTY(internal_state, SOUTH(stone))) {
	    if (libs)
      ...
      if (UNMARKED_LIBERTY(internal_state, WEST(stone))) {
	    if (libs)
      ...
      if (UNMARKED_LIBERTY(internal_state, NORTH(stone))) {
	    if (libs)
      ...
      if (UNMARKED_LIBERTY(internal_state, EAST(stone))) {
	    if (libs)
    ...
    else if (internal_state->board[pos2] == OTHER_COLOR(color)
  ...
  for (k = 0; k < captures; k++) {
     ...
     */
    if (!MARKED_COLOR_STRING(internal_state, SOUTH(lib), color)
    && !MARKED_COLOR_STRING(internal_state, WEST(lib), color)
    && !MARKED_COLOR_STRING(internal_state, NORTH(lib), color)
    && !MARKED_COLOR_STRING(internal_state, EAST(lib), color)) {
      if (libs)
	libs[liberties] = lib;
      liberties++;
      if (liberties >= maxlib)
...
count_common_libs(struct board_lib_state_struct *internal_state, int str1, int str2)
{
  ...
  ASSERT1(internal_state, IS_STONE(internal_state->board[str1]), str1);
  ASSERT1(internal_state, IS_STONE(internal_state->board[str2]), str2);
  ...
  if (liberties1 <= MAX_LIBERTIES) {
    ...
    if (!UNMARKED_LIBERTY(internal_state, libs1[k]))
	  commonlibs++;
    ...
    findlib(internal_state, str1, MAXLIBS, all_libs1);
    libs1 = all_libs1;
  }
  
  for (k = 0; k < liberties1; k++)
    if (NEIGHBOR_OF_STRING(internal_state, libs1[k], string_number[str2], internal_state->board[str2]))
      commonlibs++;
...
int
find_common_libs(struct board_lib_state_struct *internal_state, int str1, int str2, int maxlib, int *libs)
{
  int all_libs1[MAXLIBS], *libs1;
  int liberties1, liberties2;
  int commonlibs = 0;
  ...
  ASSERT1(internal_state, IS_STONE(internal_state->board[str1]), str1);
  ASSERT1(internal_state, IS_STONE(internal_state->board[str2]), str2);
  ASSERT1(internal_state, libs != NULL, str1);
...
int
findstones(struct board_lib_state_struct *internal_state, int str, int maxstones, int *stones)
{
  int s;
  int size;
  int pos;
...
int
count_adjacent_stones(struct board_lib_state_struct *internal_state, int str1, int str2, int maxstones)
{
  int s1, s2;
  int size;
  int pos;
  ...
  for (k = 0; k < size && count < maxstones; k++) {
    if (NEIGHBOR_OF_STRING(internal_state, pos, s2, internal_state->board[str2]))
      count++;
    pos = NEXT_STONE(pos);
  }
...
int 
chainlinks(struct board_lib_state_struct *internal_state, int str, int adj[MAXCHAIN])
{
...
int 
extended_chainlinks(struct board_lib_state_struct *internal_state, int str, int adj[MAXCHAIN], int both_colors)
{
  struct string_data *s;
  struct string_neighbors_data *sn;
  int n;
  ...
  for (r = 0; r < liberties; r++) {
    for (k = 0; k < 4; k++) {
      if ((internal_state->board[libs[r] + delta[k]] == OTHER_COLOR(internal_state->board[str])
       || (both_colors && internal_state->board[libs[r] + delta[k]] == internal_state->board[str]))
...
send_two_return_one(struct board_lib_state_struct *internal_state, int move, int color)
{
  ...
  for (k = 0; k < 4; k++) {
    int pos = lib + delta[k];
    if (internal_state->board[pos] == EMPTY || internal_state->board[pos] == other)
      return 0;
    if (internal_state->board[pos] == color &&
...
int
is_self_atari(struct board_lib_state_struct *internal_state, int pos, int color)
{
  int other = OTHER_COLOR(color);
  ...
  else if (internal_state->board[SOUTH(pos)] == other
  ...
  else if (internal_state->board[WEST(pos)] == other
  ...
  else if (internal_state->board[NORTH(pos)] == other
  ...
  else if (internal_state->board[EAST(pos)] == other
  ...
  return accuratelib(internal_state, pos, color, 2, NULL) <= 1;
}
...
int
liberty_of_string(struct board_lib_state_struct *internal_state, int pos, int str)
{
  ASSERT_ON_BOARD1(internal_state, pos);
  ASSERT_ON_BOARD1(internal_state, str);
  ...
  return NEIGHBOR_OF_STRING(internal_state, pos, string_number[str], internal_state->board[str]);
}
 ...
 */
int
second_order_liberty_of_string(struct board_lib_state_struct *internal_state, int pos, int str)
{
  int k;
  ASSERT_ON_BOARD1(internal_state, pos);
  ASSERT_ON_BOARD1(internal_state, str);
  ...
  for (k = 0; k < 4; k++)
    ...
    && NEIGHBOR_OF_STRING(internal_state, pos + delta[k], string_number[str], internal_state->board[str]))
      return 1;
...
int
neighbor_of_string(struct board_lib_state_struct *internal_state, int pos, int str)
{
  int color = internal_state->board[str];

  ASSERT1(internal_state, IS_STONE(color), str);
  ASSERT_ON_BOARD1(internal_state, pos);

  return NEIGHBOR_OF_STRING(internal_state, pos, string_number[str], color);
}
...
int
has_neighbor(struct board_lib_state_struct *internal_state, int pos, int color)
{
  ASSERT_ON_BOARD1(internal_state, pos);
  ASSERT1(internal_state, IS_STONE(color), pos);
...
int
adjacent_strings(struct board_lib_state_struct *internal_state, int str1, int str2)
{
  int s1, s2;
  int k;
...
int
is_ko(struct board_lib_state_struct *internal_state, int pos, int color, int *ko_pos)
{
  int other = OTHER_COLOR(color);
  int captures = 0;
  int kpos = 0;
  ...
  if (ON_BOARD(internal_state, SOUTH(pos))) {
  ...
  if (ON_BOARD(internal_state, WEST(pos))) {
  ...
  if (ON_BOARD(internal_state, NORTH(pos))) {
  ...
  if (ON_BOARD(internal_state, EAST(pos))) {
 ...
 */
int
is_ko_point(struct board_lib_state_struct *internal_state, int pos)
{
  ASSERT_ON_BOARD1(internal_state, pos);
  ...
  if (internal_state->board[pos] == EMPTY) {
    int color;
    if (ON_BOARD(internal_state, SOUTH(pos)))
      color = internal_state->board[SOUTH(pos)];
    else
      color = internal_state->board[NORTH(pos)];
    if (IS_STONE(color) && is_ko(internal_state, pos, OTHER_COLOR(color), NULL))
      return 1;
  }
  else {
    ...
    && is_ko(internal_state, sl->list[0], OTHER_COLOR(s->color), NULL))
      return 1;
  }
...
is_superko_violation(struct board_lib_state_struct *internal_state, int pos, int color, enum ko_rules type)
{
  ...
  for (k = internal_state->move_history_pointer - 1; k >= 0; k--)
    if (hashdata_is_equal(internal_state->move_history_hash[k], new_board_hash)
	&& (type == PSK
        || internal_state->move_history_color[k] == OTHER_COLOR(color)))
      return 1;
 ...
 */
int
does_capture_something(struct board_lib_state_struct *internal_state, int pos, int color)
{
  int other = OTHER_COLOR(color);

  ASSERT1(internal_state, internal_state->board[pos] == EMPTY, pos);

  if (internal_state->board[SOUTH(pos)] == other && LIBERTIES(SOUTH(pos)) == 1)
    return 1;
  
  if (internal_state->board[WEST(pos)] == other && LIBERTIES(WEST(pos)) == 1)
    return 1;
  
  if (internal_state->board[NORTH(pos)] == other && LIBERTIES(NORTH(pos)) == 1)
    return 1;
  
  if (internal_state->board[EAST(pos)] == other && LIBERTIES(EAST(pos)) == 1)
    return 1;
...
void
mark_string(struct board_lib_state_struct *internal_state, int str, signed char mx[BOARDMAX], signed char mark)
{
  int pos = str;
 ...
 */
int
move_in_stack(struct board_lib_state_struct *internal_state, int pos, int cutoff)
{
  int k;
  for (k = cutoff; k < internal_state->stackp; k++)
    if (stack[k] == pos)
      return 1;
...
void
get_move_from_stack(struct board_lib_state_struct *internal_state, int k, int *move, int *color)
{
  gg_assert(internal_state, k < internal_state->stackp);
  *move = stack[k];
  *color = move_color[k];
}
 ...
 */
int
stones_on_board(struct board_lib_state_struct *internal_state, int color)
{
...
static void
new_position(struct board_lib_state_struct *internal_state)
{
  int pos;
  int s;
  ...
  for (pos = BOARDMIN; pos < BOARDMAX; pos++) {
    if (!ON_BOARD(internal_state, pos))
      continue;
    if (IS_STONE(internal_state->board[pos]) && string_number[pos] == -1) {
      string_number[pos] = next_string;
      string[next_string].size = propagate_string(internal_state, pos, pos);
      string[next_string].color = internal_state->board[pos];
      string[next_string].origin = pos;
      string[next_string].mark = 0;
      next_string++;
      PARANOID1(internal_state, next_string < MAX_STRINGS, pos);
    }
  }
  
  /* Fill in liberty and neighbor info. */
  for (s = 0; s < next_string; s++) {
    find_liberties_and_neighbors(internal_state, s);
  }
}
...
static int
propagate_string(struct board_lib_state_struct *internal_state, int stone, int str)
{
  int size = 1;
  int k;
  ...
  for (k = 0; k < 4; k++) {
    int d = delta[k];
    if (ON_BOARD(internal_state, stone + d)
    && internal_state->board[stone + d] == internal_state->board[stone]
	&& string_number[stone + d] == -1)
      size += propagate_string(internal_state, stone + d, str);
  }
...
static void
find_liberties_and_neighbors(struct board_lib_state_struct *internal_state, int s)
{
  int pos;
  ...
  do {
    ...
    else if (UNMARKED_COLOR_STRING(internal_state, SOUTH(pos), other)) {
    ...
    else if (UNMARKED_COLOR_STRING(internal_state, WEST(pos), other)) {
    ...
    else if (UNMARKED_COLOR_STRING(internal_state, EAST(pos), other)) {
...
static void
update_liberties(struct board_lib_state_struct *internal_state, int s)
{
  int pos;
  int k;
...
static void
remove_neighbor(struct board_lib_state_struct *internal_state, int str_number, int n)
{
  int k;
  int done = 0;
...
static void
remove_liberty(struct board_lib_state_struct *internal_state, int str_number, int pos)
{
  int k;
  struct string_data *s = &string[str_number];
  ...
  if (s->liberties > MAX_LIBERTIES)
    update_liberties(internal_state, str_number);
  else {
...
static int
do_remove_string(struct board_lib_state_struct *internal_state, int s)
{
  int pos;
  int k;
  ...
  do {
    ...
    DO_REMOVE_STONE(internal_state, pos);
    pos = NEXT_STONE(pos);
  } while (!BACK_TO_FIRST_STONE(s, pos));
  ...
  if (size == 1) {
    for (k = 0; k < string[s].neighbors; k++) {
      ...
      remove_neighbor(internal_state, neighbor, s);
    ...
    for (k = 0; k < string[s].neighbors; k++) {
      ...
      if (NEIGHBOR_OF_STRING(internal_state, pos, neighbor, other)) {
	if (string[neighbor].liberties < MAX_LIBERTIES)
	  string_libs[neighbor].list[string[neighbor].liberties] = pos;
	string[neighbor].liberties++;
      }

      if (NEIGHBOR_OF_STRING(internal_state, pos2, neighbor, other)) {
	if (string[neighbor].liberties < MAX_LIBERTIES)
    ...
    for (k = 0; k < string[s].neighbors; k++) {
      remove_neighbor(internal_state, string_neighbors[s].list[k], s);
      update_liberties(internal_state, string_neighbors[s].list[k]);
    }
  }
  ...
  if (string[s].color == WHITE)
    internal_state->white_captured += size;
  else
    internal_state->black_captured += size;
 ...
 */
static void
create_new_string(struct board_lib_state_struct *internal_state, int pos)
{
  int s;
  ...
  s = next_string++;
  PARANOID1(internal_state, s < MAX_STRINGS, pos);
  string_number[pos] = s;
 ...
 */
static void
extend_neighbor_string(struct board_lib_state_struct *internal_state, int pos, int s)
{
  int k;
   ...
   */
  if (string[s].liberties > MAX_LIBERTIES) {
    update_liberties(internal_state, s);
    liberties_updated = 1;
  }
  else {
    /* The place of the new stone is no longer a liberty. */
    remove_liberty(internal_state, s, pos);
  }
  ...
  if (LIBERTY(internal_state, SOUTH(pos))) {
    ...
    && !NON_SOUTH_NEIGHBOR_OF_STRING(internal_state, SOUTH(pos), s, color))
      ADD_LIBERTY(s, SOUTH(pos));
  }
  else if (UNMARKED_COLOR_STRING(internal_state, SOUTH(pos), other)) {
    int s2 = string_number[SOUTH(pos)];
  ...
  if (LIBERTY(internal_state, WEST(pos))) {
    if (!liberties_updated
    && !NON_WEST_NEIGHBOR_OF_STRING(internal_state, WEST(pos), s, color))
      ADD_LIBERTY(s, WEST(pos));
  }
  else if (UNMARKED_COLOR_STRING(internal_state, WEST(pos), other)) {
    int s2 = string_number[WEST(pos)];
  ...
  if (LIBERTY(internal_state, NORTH(pos))) {
    if (!liberties_updated
    && !NON_NORTH_NEIGHBOR_OF_STRING(internal_state, NORTH(pos), s, color))
      ADD_LIBERTY(s, NORTH(pos));
  }
  else if (UNMARKED_COLOR_STRING(internal_state, NORTH(pos), other)) {
    int s2 = string_number[NORTH(pos)];
  ...
  if (LIBERTY(internal_state, EAST(pos))) {
    if (!liberties_updated
    && !NON_EAST_NEIGHBOR_OF_STRING(internal_state, EAST(pos), s, color))
      ADD_LIBERTY(s, EAST(pos));
  }
  else if (UNMARKED_COLOR_STRING(internal_state, EAST(pos), other)) {
    int s2 = string_number[EAST(pos)];
...
static void
assimilate_string(struct board_lib_state_struct *internal_state, int s, int pos)
{
  int k;
  int last;
  ...
  for (k = 0; k < string[s2].neighbors; k++) {
    ...
    remove_neighbor(internal_state, t, s2);
...
static void
assimilate_neighbor_strings(struct board_lib_state_struct *internal_state, int pos)
{
  int s;
  int color = internal_state->board[pos];
  int other = OTHER_COLOR(color);
  ...
  else if (UNMARKED_COLOR_STRING(internal_state, SOUTH(pos), color)) {
    assimilate_string(internal_state, s, SOUTH(pos));
  }
  ...
  else if (UNMARKED_COLOR_STRING(internal_state, WEST(pos), color)) {
    assimilate_string(internal_state, s, WEST(pos));
  }
  ...
  else if (UNMARKED_COLOR_STRING(internal_state, NORTH(pos), color)) {
    assimilate_string(internal_state, s, NORTH(pos));
  }
  ...
  else if (UNMARKED_COLOR_STRING(internal_state, EAST(pos), color)) {
    assimilate_string(internal_state, s, EAST(pos));
  }
}
...
static void
do_commit_suicide(struct board_lib_state_struct *internal_state, int pos, int color)
{
  if (internal_state->board[SOUTH(pos)] == color)
    do_remove_string(internal_state, string_number[SOUTH(pos)]);

  if (internal_state->board[WEST(pos)] == color)
    do_remove_string(internal_state, string_number[WEST(pos)]);

  if (internal_state->board[NORTH(pos)] == color)
    do_remove_string(internal_state, string_number[NORTH(pos)]);

  if (internal_state->board[EAST(pos)] == color)
    do_remove_string(internal_state, string_number[EAST(pos)]);
...
static void
do_play_move(struct board_lib_state_struct *internal_state, int pos, int color)
{
  int other = OTHER_COLOR(color);
   ...
   * stone.
   */
  DO_ADD_STONE(internal_state, pos, color);
  string_number[pos] = -1;
  ...
  if (internal_state->board[SOUTH(pos)] == color) {
    ...
    if (LIBERTIES(SOUTH(pos)) > 1) {
      remove_liberty(internal_state, string_number[SOUTH(pos)], pos);
      MARK_STRING(SOUTH(pos));
    }
    else
      captured_stones += do_remove_string(internal_state, string_number[SOUTH(pos)]);
  }

  if (UNMARKED_COLOR_STRING(internal_state, WEST(pos), color)) {
    neighbor_allies++;
    ...
    if (LIBERTIES(WEST(pos)) > 1) {
      remove_liberty(internal_state, string_number[WEST(pos)], pos);
      MARK_STRING(WEST(pos));
    }
    else
      captured_stones += do_remove_string(internal_state, string_number[WEST(pos)]);
  }

  if (UNMARKED_COLOR_STRING(internal_state, NORTH(pos), color)) {
    neighbor_allies++;
    ...
    if (LIBERTIES(NORTH(pos)) > 1) {
      remove_liberty(internal_state, string_number[NORTH(pos)], pos);
      MARK_STRING(NORTH(pos));
    }
    else
      captured_stones += do_remove_string(internal_state, string_number[NORTH(pos)]);
  }

  if (UNMARKED_COLOR_STRING(internal_state, EAST(pos), color)) {
    neighbor_allies++;
    ...
    if (LIBERTIES(EAST(pos)) > 1) {
      remove_liberty(internal_state, string_number[EAST(pos)], pos);
#if 0
      MARK_STRING(EAST(pos));
#endif
    }
    else
      captured_stones += do_remove_string(internal_state, string_number[EAST(pos)]);
  }

  /* Choose strategy depending on the number of friendly neighbors. */
  if (neighbor_allies == 0)
    create_new_string(internal_state, pos);
  else if (neighbor_allies == 1) {
    gg_assert(internal_state, s >= 0);
    extend_neighbor_string(internal_state, pos, s);
    return; /* can't be a ko, we're done */
  }
  else {
    assimilate_neighbor_strings(internal_state, pos);
    return; /* can't be a ko, we're done */
  }
...
#else
#define code1(internal_state, arg) \
  if (!ON_BOARD(internal_state, arg)) \
    (*number_edges)++; \
  else if (internal_state->board[arg] == EMPTY) \
    (*number_open)++; \
  else { \
    int s = string_number[arg]; \
    if (string_number[str] == s) \
      (*number_same_string)++; \
    if (internal_state->board[arg] == color) { \
      (*number_own)++; \
      if (string[s].liberties == 1) \
	(*saved_stones) += string[s].size; \
    } \
  ...
  code1(internal_state, SOUTH(move));
  code1(internal_state, WEST(move));
  code1(internal_state, NORTH(move));
  code1(internal_state, EAST(move));
#endif
}
