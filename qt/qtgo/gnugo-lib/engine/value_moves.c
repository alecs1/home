 ...
 */
static int
move_connects_strings(board_lib_state_struct *internal_state,
                      int pos, int color, int to_move)
{
  int ss[4];
  int strings = 0;
  int own_strings = 0;
  ...
  for (k = 0; k < strings; k++) {
	...
	int dummy[MAXCHAIN];
	fewlibs++;
    fewlibs += chainlinks2(internal_state, ss[k], dummy, 1);
      }
    }
  }
  ...
  if (fewlibs > 4)
    fewlibs = 4;
  if (to_move && is_ko(internal_state, pos, color, NULL) && fewlibs > 1)
    fewlibs = 1;
                             ...
                             int move, int color)
{
  ...
  return blunder_size(internal_state, move, color, NULL, safe_stones);
}
  ...
  for (ii = BOARDMIN; ii < BOARDMAX; ii++) {
    ...
    if (internal_state->board[aa] == color
	  ...
	  if (acode < worm[aa].attack_codes[0]) {
		...
		int this_acode = REVERSE_RESULT(find_defense(internal_state, aa, NULL));
		if (this_acode > acode) {
		  acode = this_acode;
		  if (acode >= worm[aa].attack_codes[0])
		    defense_works = 0;
					  ...
					  int move_reason_type, int what)
{
  ...
  for (k = 0; k < 2; k++) {
    ...
    if ((move_reason_type == STRATEGIC_ATTACK_MOVE
	 ...
	 || move_reason_type == ATTACK_MOVE_BAD_KO
	 || (move_reason_type == VITAL_EYE_MOVE
         && internal_state->board[dd] == OTHER_COLOR(color)))
    && !owl_attack_move_reason_known(internal_state, pos, dd)) {
      int kworm = NO_MOVE;
      int acode = owl_does_attack(internal_state, pos, dd, &kworm);
    ...
    if ((move_reason_type == STRATEGIC_DEFEND_MOVE
	 ...
	 || move_reason_type == DEFEND_MOVE_BAD_KO
	 || (move_reason_type == VITAL_EYE_MOVE
         && internal_state->board[dd] == color))
    && !owl_defense_move_reason_known(internal_state, pos, dd)) {
      int kworm = NO_MOVE;
      /* FIXME: Better use owl_connection_defend() for CONNECT_MOVE ? */
      int dcode = owl_does_defend(internal_state, pos, dd, &kworm);
                           ...
                           int pos, int color, int target, int dist)
{
  ...
  ASSERT1(internal_state, internal_state->board[target] == OTHER_COLOR(color), pos);
  ASSERT1(internal_state, !owl_attack_move_reason_known(internal_state, pos, target), pos);
  DEBUG(internal_state, DEBUG_LARGE_SCALE, "Trying large scale move %1m on %1m\n", pos, target);
  ...
  if (DRAGON2(target).owl_attack_node_count < owl_node_limit) {
    ...
    acode = owl_does_attack(internal_state, pos, target, &kworm);
	...
	&& acode == WIN) {
      ...
      DEBUG(internal_state, DEBUG_LARGE_SCALE | DEBUG_MOVE_REASONS,
	    "Move at %1m owl-attacks %1m on a large scale(%s).\n", 
	    pos, target, result_to_string(acode));
    }
    else
      DEBUG(internal_state, DEBUG_LARGE_SCALE,
	    "Move at %1m isn't a clean large scale attack on %1m (%s).\n",
	    pos, target, result_to_string(acode));
    
    DEBUG(internal_state, DEBUG_LARGE_SCALE, "  owl nodes used = %d, dist = %d\n", 
	  owl_nodes_used, dist);
    /* Restore settings. */
    verbose = save_verbose;
  }
                                       ...
                                       int color, int target)
{
  ...
  int x_max = 0;
  int y_min = internal_state->board_size;
  int y_max = 0;
  int dist;

  ASSERT1(internal_state, internal_state->board[target] == OTHER_COLOR(color), target);
  ...
  for (dist = 0; dist <= MAXIMUM_LARGE_SCALE_DIST; dist++)
    for (x = gg_max(x_min - dist, 0);
     x <= gg_min(x_max + dist, internal_state->board_size - 1); x++)
      for (y = gg_max(y_min - dist, 0);
       y <= gg_min(y_max + dist, internal_state->board_size - 1); y++) {
	int pos = POS(x, y);
    ASSERT1(internal_state, ON_BOARD2(internal_state, x, y), pos);
    ...
    if (internal_state->board[pos] == EMPTY) {
	  ...
	  if (gg_max(dx, dy) == dist  
	      && move[pos].reason[0] >= 0
          && !owl_attack_move_reason_known(internal_state, pos, target))
	    /* Maximum Manhatan distance, move reason known but no owl
	     * attack yet.
	     */
	    try_large_scale_owl_attack(internal_state, pos, color, target, dist);
                                       ...
                                       int color)
{
  ...
  for (pos = BOARDMIN; pos < BOARDMAX; pos++) {
	...
	&& our_vital_points[pos].defense_points[0] != NO_MOVE) {
      int k, dr;
      find_eye_dragons(internal_state, pos, our_eyes, color, &dr, 1);
	...
	&& your_vital_points[pos].attack_points[0] != NO_MOVE) {
      int k, dr;
      find_eye_dragons(internal_state, pos, your_eyes, OTHER_COLOR(color), &dr, 1);
      for (k = 0; k < MAX_EYE_ATTACKS; k++) {
  ...
  for (pos = BOARDMIN; pos < BOARDMAX; pos++) {
	...
	if (worth_trying) {
      if (internal_state->board[pos] == color
          && !owl_defense_move_reason_known(internal_state, pos2, pos)) {
	    int kworm = NO_MOVE;
        ...
        add_owl_defense_move(internal_state, pos2, pos, dcode);
	      if (save_verbose)
           ...
           && !owl_attack_move_reason_known(internal_state, pos2, pos)) {
	    int kworm = NO_MOVE;
	    int acode = owl_does_attack(internal_state, pos2, pos, &kworm);
                          ...
                          int pos, int color, struct move_reason *reason)
{
  ...
  if (resulta == 0 && resultb == 0
    ...
    DEBUG(internal_state, DEBUG_MOVE_REASONS, "Potential semeai move at %1m for %1m discarded\n",
	  pos, dr1);
}
                       ...
                       int color)
{
  ...
  for (pos = BOARDMIN; pos < BOARDMAX; pos++) {
    ...
    if (!ON_BOARD1(internal_state, pos))
      continue;
    for (k = 0; k < MAX_REASONS; k++) {
      r = move[pos].reason[k];
      if (r < 0)
                              ...
                              int color)
{
  ...
  for (pos = BOARDMIN; pos < BOARDMAX; pos++) {
	...
	if (move_reasons[r].type == ATTACK_MOVE) {
	  attack_move = 1;
      color_to_move = OTHER_COLOR(internal_state->board[aa]);
	}
	else {
	  attack_move = 0;
      color_to_move = internal_state->board[aa];
	}
    ...
    num_adj = extended_chainlinks(internal_state, aa, adjs, 1);
	...
	for (i = 0; i < num_adj; i++) {
	  for (j = i+1; j < num_adj; j++) {
        ...
        if (internal_state->board[adj1] != internal_state->board[adj2])
	      continue;
	    if (attack_move
        && internal_state->board[adj1] != internal_state->board[aa]
		&& !disconnect(internal_state, adj1, adj2, NULL))
	      continue;
	    if (!attack_move
        && internal_state->board[adj1] != internal_state->board[aa]
		&& !string_connect(internal_state, adj1, adj2, NULL))
	      continue;
	    if (attack_move
        && internal_state->board[adj1] == internal_state->board[aa])
	      continue;
        ...
        if (trymove(internal_state, pos, color_to_move, "induce_secondary_move_reasons",
			aa)) {
	      if (attack_move
          && internal_state->board[adj1] != internal_state->board[aa]
		  && !disconnect(internal_state, adj1, adj2, NULL)) {
	...
	for (pos2 = BOARDMIN; pos2 < BOARDMAX; pos2++) {
      if (ON_BOARD(internal_state, pos2) && internal_state->board[pos2] == EMPTY
	    ...
	    for (j = 0; j < 8; j++) {
          ...
          if (ON_BOARD(internal_state, pos3) && internal_state->board[pos3] == color
                    ...
                    int color)
{
  ...
  for (pos = BOARDMIN; pos < BOARDMAX; pos++) {
    ...
    if (!ON_BOARD(internal_state, pos))
      continue;
    tactical_safety = is_known_safe_move(internal_state, pos);
      ...
      case OWL_DEFEND_MOVE_LOSS:
	{
	  ...
	  for (ii = first_worm_in_dragon(what); ii != NO_MOVE; 
	    ...
	    if (!play_connect_n(internal_state, color, 0, 1, pos, ii, pos))
	      break;
	  }
	  if (ii != NO_MOVE) {
	    tactical_safety = 1;
	...
	break;
	  ...
	  if (type == ATTACK_MOVE
        ...
        num_adj = chainlinks(internal_state, aa, adjs);
	    for (m = 0; m < num_adj; m++) {
	      int adj = adjs[m];
	      
          if (internal_state->board[adj] == color) {
	  ...
	  for (m = 0; m < 4; m++) {
	    int d = delta[m];
        if (internal_state->board[pos+d] == color) {
	      bb = pos + d;
	      break;
	    }
	  }
	   ...
	   */
	  if (owl_does_defend(internal_state, pos, bb, NULL)) {
	    tactical_safety = 1;
	    safety = 1;
	  }
	  break;
      ...
      case DEFEND_MOVE_BAD_KO:
	{
	  ...
	  else if (!play_connect_n(internal_state, color, 0, 1, pos, aa, pos)
		   && owl_does_defend(internal_state, pos, aa, NULL))
	    safety = 1;
	  break;
	}
	...
	break;
      ...
      if (safety == 1 && (tactical_safety == 1 || safe_move(internal_state, pos, color)))
	break;
    }
      
    if (safety == 1 && (tactical_safety || safe_move(internal_state, pos, color)))
                 ...
                 int dragona, int dragonb, int tt, float margin)
{
  ...
  float crude_weakness_a
    = crude_dragon_weakness(internal_state, da->safety, &da->genus, da->lunch != NO_MOVE,
                           ...
                           int pos, int ww)
{
  ...
  for (s = 0; s < num_adj; s++) {
	...
	&& !does_defend(internal_state, pos, adj)
                  ...
                  int pos, int color)
{
  ...
  for (k = 0; k < MAX_REASONS; k++) {
	...
	for (i = 0; i < 4; i++)
      if (internal_state->board[target + delta[i]] == OTHER_COLOR(color)) {
                          ...
                          int pos, int color)
{
  ...
  int saved_stackp = internal_state->stackp;
  int result = 0;
  ...
  if (trymove(internal_state, pos, color, NULL, EMPTY)) {
    for (k = 0; k < 12; k++) {
      int pos2;
      if (k < 8)
	pos2 = pos + delta[k];
      else if (ON_BOARD(internal_state, pos + delta[k - 8]))
			   ...
			   int disable_delta_territory_cache)
{
  ...
  for (k = 0; k < MAX_REASONS; k++) {
    ...
    case ATTACK_MOVE_BAD_KO:
      ...
      ASSERT1(internal_state, internal_state->board[aa] != color, aa);
	...
	break;
      ...
      mark_changed_string(internal_state, aa, safe_stones, strength, 0);
      TRACE(internal_state, "  %1m: attack on worm %1m\n", pos, aa);
	...
	this_value *= 0.3;
    TRACE(internal_state, "  %1m: -%f - attack on worm %1m only with good ko\n",
	      pos, this_value, aa);
      }	
      else if (move_reasons[r].type == ATTACK_MOVE_BAD_KO) {
	this_value *= 0.5;
    TRACE(internal_state, "  %1m: -%f - attack on worm %1m only with bad ko\n",
	      pos, this_value, aa);
      }
	...
	break;
      ...
      && (owl_defense_move_reason_known(internal_state, pos, aa)
          < defense_move_reason_known(internal_state, pos, aa))
      && !semeai_move_reason_known(internal_state, pos, aa)) {
	DEBUG(internal_state, DEBUG_MOVE_REASONS,
	...
	break;
      ...
      mark_changed_string(internal_state, aa, safe_stones, strength, INFLUENCE_SAVED_STONE);
      TRACE(internal_state, "  %1m: defense of worm %1m\n", pos, aa);
	...
	this_value *= 0.3;
    TRACE(internal_state, "  %1m: -%f - defense of worm %1m with good ko\n",
	      pos, this_value, aa);
      }	
      else if (move_reasons[r].type == DEFEND_MOVE_BAD_KO) {
	this_value *= 0.5;
    TRACE(internal_state, "  %1m: -%f - defense of worm %1m with bad ko\n",
	      pos, this_value, aa);
      }	
    ...
    TRACE(internal_state, "  %1m: -%f - suspected ineffective defense of worm %1m\n",
	      pos, this_value, aa);
	tot_value -= this_value;
      }
    ...
    case ATTACK_THREAT:
      ...
      ASSERT1(internal_state, internal_state->board[aa] == other, aa);
	...
	break;
      ...
      if (trymove(internal_state, pos, color, "estimate_territorial_value-A", NO_MOVE)) {
	int adjs[MAXCHAIN];
	 ...
	 */
    if (internal_state->board[aa] == EMPTY)
	  num_adj = 0;
	else
      num_adj = chainlinks(internal_state, aa, adjs);
    ...
    if (!is_known_good_attack_threat(internal_state, pos, aa)
	  ...
	  if (attack(internal_state, pos, &attack_move) != WIN) {
	    int i;

        if (trymove(internal_state, defense_move, other,
			"estimate_territorial_value-b", NO_MOVE)) {
          if (internal_state->board[pos] == EMPTY || attack(internal_state, pos, NULL) != 0) {
        popgo(internal_state);
        popgo(internal_state);
		break;
	      }
		...
		if (move_reasons[reason].type == ATTACK_MOVE
            && internal_state->board[attacked_string] == other) {
		  int defense_code = find_defense(internal_state, attacked_string, NULL);
	     ...
	     * string?
	     */
        if (trymove(internal_state, attack_move, other,
			"estimate_territorial_value-c", NO_MOVE)) {
	      if (attack(internal_state, aa, NULL) == 0) {
	     ...
	     * string.
	     */
        adjustment_down = 2 * countstones(internal_state, pos);
	  }

	  bad_followup = 0;
	  for (s = 0; s < num_adj; s++) {
	    int lib;
        if (countlib(internal_state, adjs[s]) == 1) {
          findlib(internal_state, adjs[s], 1, &lib);
          if (trymove(internal_state, lib, other,
		    	  "estimate_territorial_value-d", NO_MOVE)) {
		if (!attack(internal_state, aa, NULL)
            && (internal_state->board[pos] == EMPTY || attack(internal_state, pos, NULL) != 0)) {
          popgo(internal_state);
		  bad_followup = 1;
		  break;
		}
	...
	for (s = 0; s < num_adj; s++) {
	  ...
	  if (dragon[adj].color == color
	    ...
	    adjustment_up = 2*dragon[adj].effective_size;
	  if (dragon[adj].color == color
	      && attack(internal_state, adj, NULL)
	      && 2*worm[adj].effective_size > adjustment_down)
	    adjustment_down = 2*worm[adj].effective_size;
	}
	...
	if (move[pos].move_safety == 1
	    && attack(internal_state, aa, &attack_move) == WIN
	    && attack_move != NO_MOVE) {
      if (trymove(internal_state, attack_move, other,
		      "estimate_territorial_value-b", NO_MOVE)) {
        if (internal_state->board[pos] == EMPTY || attack(internal_state, pos, NULL) != 0) {
	...
	tot_value -= 0.5;
      ...
      TRACE(internal_state, "  %1m: owl attack/defend for %1m\n", pos, aa);
	...
	this_value = 0.0;
	       ...
	       || move_reasons[r].type == OWL_DEFEND_MOVE_GOOD_KO) {
	this_value *= 0.3;
    TRACE(internal_state, "  %1m: -%f - owl attack/defense of %1m only with good ko\n",
	      pos, this_value, aa);
      }	
      else if (move_reasons[r].type == OWL_ATTACK_MOVE_BAD_KO
	       || move_reasons[r].type == OWL_DEFEND_MOVE_BAD_KO) {
	this_value *= 0.5;
    TRACE(internal_state, "  %1m: -%f - owl attack/defense of %1m only with bad ko\n",
	      pos, this_value, aa);
      }
	...
	this_value = 0.0;
      ...
      && attack_move_reason_known(internal_state, pos, aa) != WIN) {
	if (large_scale)
	  this_value = (2.0 + 0.05 * (2 * worm[aa].effective_size));
	else
	  this_value = 0.05 * (2 * worm[aa].effective_size);
    TRACE(internal_state, "  %1m: -%f - suspected ineffective owl attack of worm %1m\n",
	      pos, this_value, aa);
	tot_value -= this_value;
      }
	...
	for (s = 0; s < DRAGON2(aa).neighbors; s++) {
	  ...
	  if (dragon[adj].color == color
          ...
          && !owl_defense_move_reason_known(internal_state, pos, adj))
	    value = 0.0;
	}
    ...
    if (internal_state->board[aa] == color)
      ...
      TRACE(internal_state, "  %1m: %f (reverse followup) - prevent threat to defend %1m\n",
		pos, 2 * dragon[aa].effective_size, aa);
      }
      break;
    ...
    case MY_ATARI_ATARI_MOVE:
      ...
      tot_value += this_value;
      TRACE(internal_state, "  %1m: %f - combination attack kills one of several worms\n",
	    pos, this_value);
      break;
    ...
    case YOUR_ATARI_ATARI_MOVE:
      ...
      tot_value += this_value;
      TRACE(internal_state, "  %1m: %f - defends against combination attack on several worms\n",
	    pos, this_value);
      break;
    }
  }
  ...
  if (move[pos].move_safety == 1
    ...
    if (0)
      TRACE(internal_state, "  %1m: is a safe move\n", pos);
  }
  else {
    TRACE(internal_state, "  %1m: not a safe move\n", pos);
    safe_stones[pos] = 0;
    strength[pos] = 0.0;
  }
  ...
  if (does_block
    ...
    || !retrieve_delta_territory_cache(internal_state, pos, color, &this_value,
      ...
      if (this_value != 0.0)
    TRACE(internal_state, "%1m: %f - change in territory\n", pos, this_value);
      else
    ...
    = influence_delta_territory(internal_state, &move_influence, &followup_influence,
      ...
      if (this_value != 0.0)
    TRACE(internal_state, "%1m: %f - change in territory (cached)\n", pos, this_value);
      else
	DEBUG(internal_state, DEBUG_MOVE_REASONS,
  ...
  if (tot_value < move[pos].min_territory
    ...
    TRACE(internal_state, "  %1m:   %f - revised to meet minimum territory value\n",
	  pos, tot_value);
  }
  if (tot_value > move[pos].max_territory) {
    tot_value = move[pos].max_territory;
    TRACE(internal_state, "  %1m:   %f - revised to meet maximum territory value\n",
	  pos, tot_value);
  }
    			   ...
    			   int use_thrashing_dragon_heuristics)
{
  ...
  for (k = 0; k < MAX_REASONS; k++) {
	...
	if (worm[aa].cutstone2 > 1 && !worm[aa].inessential) {
      ...
      TRACE(internal_state, "  %1m: %f - %1m cutstone\n", pos, this_value, aa);
	}
	...
	for (l = 0; l < next_lunch; l++)
	  if (lunch_worm[l] == aa) {
        ...
        && (attack_move_reason_known(internal_state, pos, bb)
            || defense_move_reason_known(internal_state, pos, bb)))
	      this_value = 0.0;
        ...
        && ((color == internal_state->board[bb] && !does_defend(internal_state, pos, bb))
            || (color == OTHER_COLOR(internal_state->board[bb])
			&& !does_attack(internal_state, pos, bb))))
	      this_value = 0.0;
	 ...
	 * the same thing.
	 */
    if (move_reason_known(internal_state, pos, YOUR_ATARI_ATARI_MOVE, -1))
	  break;
    ...
    TRACE(internal_state, "  %1m: %f - both defends %1m (%f) and defends %1m (%f)\n",
	      pos, this_value, aa, aa_value, bb, bb_value);
  ...
  for (aa = BOARDMIN; aa < BOARDMAX; aa++) {
    ...
    ASSERT1(internal_state, dragon[aa].origin == aa, aa);
    ...
    && !owl_move_reason_known(internal_state, pos, aa)) {
      DEBUG(internal_state, DEBUG_MOVE_REASONS, "  %1m: 0.0 - disregarding strategic effect on %1m (critical dragon)\n",
	    pos, aa);
      continue;
    }
    ...
    && (attack_move_reason_known(internal_state, pos, aa)
        || defense_move_reason_known(internal_state, pos, aa))) {
      TRACE(internal_state, "  %1m:   %f - %1m strategic value already counted - A.\n",
     ...
     * territorial value.
     */
    if (attack_move_reason_known(internal_state, pos, aa)
    || defense_move_reason_known(internal_state, pos, aa)
    || (owl_move_reason_known(internal_state, pos, aa)
	    && dragon[aa].status == CRITICAL)
    || move_reason_known(internal_state, pos, SEMEAI_MOVE, aa)) {
      ...
      if (excess_value > 0.0) {
    TRACE(internal_state, "  %1m: %f - strategic bonus for %1m\n", pos, excess_value, aa);
	tot_value += excess_value;
      }
      else {
    TRACE(internal_state, "  %1m:   %f - %1m strategic value already counted - B.\n",
	      pos, dragon_value[aa], aa);
      }
    ...
    TRACE(internal_state, "  %1m: %f - strategic effect on %1m\n",
	  pos, dragon_value[aa], aa);
  ...
  if (this_value > 0.0) {
    TRACE(internal_state, "  %1m: %f - strategic penalty, considered as invasion.\n",
	  pos, -this_value);
    tot_value -= this_value;
  }
		   ...
		   float our_score, int use_thrashing_dragon_heuristics)
{
  ...
  if (tot_value > 0.0) {
    ...
    followup_value = move[pos].followup_value
		     + move[pos].influence_followup_value;
    TRACE(internal_state, "  %1m:   %f - total followup value, added %f as territorial followup\n",
    ...
    if (tot_value >= 0.5) {
      ...
      if (contribution != 0.0) {
    TRACE(internal_state, "  %1m: %f - added due to followup (%f) and reverse followup values (%f)\n",
    ...
    if (move[pos].secondary_value != 0.0)
      TRACE(internal_state, "  %1m: %f - secondary\n", pos,
    ...
    gprintf(internal_state, "  %1m: %f - shape ", pos, tot_value - old_value);
	fprintf(stderr,
    ...
    if (c > 0) {
      ...
      if (verbose) {
	/* Should all have been TRACE, except we want field sizes. */
    gprintf(internal_state, "  %1m: %f - connects strings ", pos,
     ...
     */
    if (tot_value < 0.3
    && (move_reason_known(internal_state, pos, CONNECT_MOVE, -1)
        || move_reason_known(internal_state, pos, CUT_MOVE, -1))) {
      float old_tot_value = tot_value;
      tot_value = gg_min(0.3, tot_value + 0.1);
      TRACE(internal_state, "  %1m: %f - cut/connect dame bonus\n", pos,
	    tot_value - old_tot_value);
    }
  }
  else {
  ...
  if (pure_threat_value > 0.0 
    ...
    if (new_tot_value > tot_value) {
      TRACE(internal_state, "  %1m: %f - carry out threat or defend against threat\n",
  ...
  if (tot_value < move[pos].min_value
    ...
    TRACE(internal_state, "  %1m:   %f - minimum accepted value\n", pos, tot_value);
  }
  
  if (tot_value > move[pos].max_value) {
    tot_value = move[pos].max_value;
    TRACE(internal_state, "  %1m:   %f - maximum accepted value\n",
          pos, tot_value);
  }
  ...
  if (tot_value > 0
    ...
    TRACE(internal_state, "Move generation values %1m to %f\n", pos, tot_value);
  ...
  for (n = 0; n < internal_state->board_size; n++)
    for (m = internal_state->board_size-1; m >= 0; m--) {
      ...
      if (is_legal(internal_state, pos, color) || is_illegal_ko_capture(internal_state, pos, color)) {
        ...
        move[pos].value = 0.0;
        TRACE(internal_state, "Move at %1m wasn't legal.\n", pos);
      }
    }
}
                      ...
                      FILE *output)
{
  ...
  for (pos = BOARDMIN; pos < BOARDMAX; pos++) {
    ...
    gfprintf(internal_state, output, "%1M %f\n", pos, move[pos].final_value);
  }
}
                      ...
                      int ko_move, int color, float ko_value)
{
  ...
  TRACE(internal_state, "Reevaluating ko threats.\n");
  for (pos = BOARDMIN; pos < BOARDMAX; pos++) {
    int threat_quality = 0;

    if (!ON_BOARD(internal_state, pos) || pos == ko_move)
      continue;
    if (move[pos].additional_ko_value <= 0.0) 
      continue;
    ...
    if (!find_defense(internal_state, ko_stone, &opp_ko_move))
      ...
      if (trymove(internal_state, opp_ko_move, OTHER_COLOR(color),
	    ...
	    if (threat_does_work && type == ATTACK_THREAT) {
	      int apos;
	      if (attack(internal_state, pos, &apos)
		  && does_defend(internal_state, apos, what)
                ...
                int color, float our_score, float game_status)
{
  ...
  TRACE(internal_state, "  Game status = %f (0.0 = start, 1.0 = game over)\n", game_status);
  ...
  if (cosmic_gnugo) {
    ...
    if (game_status > 0.65 && our_score > 15.0) {
      ...
      followup_weight       = 1.1;
      TRACE(internal_state, "  %s is leading, using conservative settings.\n",
      ...
      TRACE(internal_state, "  %s is not winning enough, using aggressive settings.\n",
	    color == WHITE ? "White" : "Black");
    }
  }
}
