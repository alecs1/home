...
void 
vgprintf(struct board_lib_state_struct *internal_state, FILE *outputfile, const char *fmt, va_list ap)
{
  if (fmt[0] == '%' && fmt[1] == 'o')
    fmt += 2;  /* cancel indent */
  else if (internal_state->stackp > 0)
    fprintf(outputfile, "%.*s", internal_state->stackp*2, "                                ");
  ...
  for (; *fmt; ++fmt) {
	...
	if (m == -1 && n == -1)
    ...
    else if (!ON_BOARD2(internal_state, m, n))
	  ...
	  if (n < 8)
	    ...
	    movename[0] = n + 66;
	  if (*fmt == 'm')
        sprintf(movename+1, "%d", internal_state->board_size - m);
	  else
        sprintf(movename+1, "%-2d", internal_state->board_size - m);
	...
	if (pos == NO_MOVE)
	  fputs("PASS", outputfile);
    else if (!ON_BOARD1(internal_state, pos))
    ...
    fputs(color_to_string(internal_state, color), outputfile);
	break;
      }
      default:
...
gfprintf(struct board_lib_state_struct *internal_state, FILE *outfile, const char *fmt, ...)
{
  va_list ap;
  va_start(ap, fmt);
  vgprintf(internal_state, outfile, fmt, ap);
  va_end(ap);
}
...
int 
gprintf(struct board_lib_state_struct *internal_state, const char *fmt, ...)
{
  va_list ap;
  va_start(ap, fmt);
...
void
mprintf(struct board_lib_state_struct *internal_state, const char *fmt, ...)
{
  va_list ap;
  va_start(ap, fmt);
  vgprintf(internal_state, stdout, fmt, ap);
  va_end(ap);
}
 ...
 */
static void
dump_board_sgf(struct board_lib_state_struct *internal_state)
{
  int pos;
  ...
  for (pos = BOARDMIN; pos < BOARDMAX; pos++)
    if (ON_BOARD(internal_state, pos))
      initial_colors_found |= internal_state->initial_board[pos];
  
  fprintf(stderr, "(;GM[1]FF[4]SZ[%d]KM[%.1f]HA[%d]GN[GNU Go %s stepped on a bug]\n",
      internal_state->board_size, internal_state->komi, internal_state->handicap, gg_version());

  for (color = WHITE; color <= BLACK; color++) {
    if (initial_colors_found & color) {
      fprintf(stderr, "A%s", color == WHITE ? "W" : "B");
      for (k = 0, pos = BOARDMIN; pos < BOARDMAX; pos++) {
    if (ON_BOARD(internal_state, pos) && internal_state->initial_board[pos] == color) {
  ...
  if (internal_state->move_history_pointer > 0) {
    for (k = 0; k < internal_state->move_history_pointer; k++) {
      fprintf(stderr, ";%s", internal_state->move_history_color[k] == WHITE ? "W" : "B");
      if (internal_state->move_history_pos[k] == PASS_MOVE)
	fprintf(stderr, "[]");
      else
    fprintf(stderr, "[%c%c]", 'a' + J(internal_state->move_history_pos[k]),
        'a' + I(internal_state->move_history_pos[k]));
...
const char *
color_to_string(struct board_lib_state_struct *internal_state, int color)
{
  gg_assert(internal_state, color < NUM_KOMASTER_STATES);
  return color_names[color];
}

/* Convert a location to a string. */
const char *
location_to_string(struct board_lib_state_struct *internal_state, int pos)
{
  static int init = 0;
  ...
  if (!init) {
    ...
    for (pos = 0; pos < BOARDSIZE; pos++)
      location_to_buffer(internal_state, pos, buf[pos]);
    init = 1;
  }
  ASSERT1(internal_state, pos >= 0 && pos < BOARDSIZE, pos);
  return buf[pos];
}
...
void
location_to_buffer(struct board_lib_state_struct *internal_state, int pos, char *buf)
{
  char *bufp = buf;
  int i = I(pos);
  int j = J(pos);
...
int
is_hoshi_point(struct board_lib_state_struct *internal_state, int m, int n)
{
  int hoshi;
  int middle;

  /* No hoshi points on these boards. */
  if (internal_state->board_size == 2 || internal_state->board_size == 4)
    return 0;

  /* In the middle of a 3x3 board. */
  if (internal_state->board_size == 3) {
    if (m == 1 && n == 1)
      return 1;
  ...
  if (internal_state->board_size == 5) {
    if (m == 1 && (n == 1 || n == 3))
      return 1;
    if (m == 2 && n == 2)
      return 1;
  ...
  middle = internal_state->board_size/2;
    
  /* Normalize the coordinates by mirroring to the lower numbers. */
  if (m >= middle)
    m = internal_state->board_size - 1 - m;
  if (n >= middle)
    n = internal_state->board_size - 1 - n;
  ...
  if (internal_state->board_size < 12) {
 ...
 */
void
simple_showboard(struct board_lib_state_struct *internal_state, FILE *outfile)
{
  int i, j;
