...
void 
hashdata_calc_orientation_invariant(struct board_lib_state_struct *internal_state, Hash_data *hd, Intersection *p, int ko_pos)
{
  int pos;
  int rot;
  Hash_data hd_rot;
  ...
  for (rot = 0; rot < 8; rot++) {
    ...
    for (pos = BOARDMIN; pos < BOARDMAX; pos++) {
      if (p[pos] == WHITE)
    hashdata_xor(hd_rot, white_hash[rotate1(internal_state, pos, rot)]);
      else if (p[pos] == BLACK)
    hashdata_xor(hd_rot, black_hash[rotate1(internal_state, pos, rot)]);
    }
    
    if (ko_pos != NO_MOVE)
      hashdata_xor(hd_rot, ko_hash[rotate1(internal_state, ko_pos, rot)]);
...
char *
hashdata_to_string(struct board_lib_state_struct *internal_state, Hash_data *hashdata)
{
  ...
  for (k = NUM_HASHVALUES - 1; k >= 0; k--) {
    ...
    gg_assert(internal_state, n < BUFFER_SIZE);
  }
