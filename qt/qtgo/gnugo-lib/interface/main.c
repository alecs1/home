...
main(int argc, char *argv[])
{
   ...
   */
                               ...
                               long_options, NULL)) != EOF)
    {
        ...
        case OPT_FORBID_SUICIDE:
            internal_state->suicide_rule = FORBIDDEN;
            break;
        ...
        case OPT_ALLOW_ALL_SUICIDE:
            internal_state->suicide_rule = ALL_ALLOWED;
            break;

        case OPT_SIMPLE_KO:
            internal_state->ko_rule = SIMPLE;
            break;

        case OPT_NO_KO:
            internal_state->ko_rule = NONE;
            break;

        case OPT_POSITIONAL_SUPERKO:
            internal_state->ko_rule = PSK;
            break;

        case OPT_SITUATIONAL_SUPERKO:
            internal_state->ko_rule = SSK;
            break;
        ...
        case OPT_LIMIT_SEARCH:
        {
            ...
            set_search_diamond(internal_state, pos);
        }
            break;
    ...
    if (requested_boardsize != -1) {
        if (!check_boardsize(requested_boardsize, stderr))
            exit(EXIT_FAILURE);
        gnugo_clear_board(internal_state, requested_boardsize);
    }
    ...
    init_gnugo(internal_state, memory, seed);
   ...
   */
    if (strlen(mc_pattern_filename) > 0) {
        if (!mc_load_patterns_from_db(internal_state, mc_pattern_filename, NULL))
            return EXIT_FAILURE;
    }
    else if (strlen(mc_pattern_name) > 0) {
        if (!choose_mc_patterns(internal_state, mc_pattern_name)) {
    ...
    if (infilename) {
        ...
        if (gameinfo_play_sgftree_rot(internal_state, &gameinfo, &sgftree, untilstring,
                                      orientation) == EMPTY) {
   ...
   */
    ...
    case MODE_SGMP:
        ...
        play_gmp(internal_state, &gameinfo, playmode == MODE_SGMP);
    ...
    case MODE_REPLAY:
        ...
        play_replay(internal_state, &sgftree, replay_color);
        break;
    ...
    case MODE_LOAD_AND_ANALYZE:
        ...
        load_and_analyze_sgf_file(internal_state, &gameinfo);
    ...
    case MODE_LOAD_AND_SCORE:
        ...
        load_and_score_sgf_file(internal_state, &sgftree, &gameinfo, scoringmode);
        break;
    ...
    case MODE_LOAD_AND_PRINT:
        if (!infilename) {
            ...
            sgffile_printsgf(internal_state, gameinfo.to_move, printsgffile);
        }
        break;
    ...
    case MODE_DECIDE_STRING:
    {
        ...
        decide_string(internal_state, str);
    }
        break;
    ...
    case MODE_DECIDE_CONNECTION:
    {
        ...
        decide_connection(internal_state, str1, str2);
    }
        break;
    ...
    case MODE_DECIDE_DRAGON_DATA:
    {
        ...
        decide_dragon_data(internal_state, pos);
    }
        break;
    ...
    case MODE_DECIDE_SEMEAI:
    {
        ...
        decide_semeai(internal_state, pos1, pos2);
    }
        break;
    ...
    case MODE_DECIDE_POSITION:
    {
        ...
        decide_position(internal_state);
    }
        break;
    ...
    case MODE_DECIDE_COMBINATION:
    {
        ...
        decide_combination(internal_state, color);
    }
        break;
    ...
    case MODE_DECIDE_SURROUNDED:
    {
        ...
        decide_surrounded(internal_state, pos);
        break;
    }
    ...
    case MODE_GTP:
    {
        ...
        printf("play_gtk is currently disabled\n");
        //play_gtp(internal_state, gtp_input_FILE, gtp_output_FILE, gtp_dump_commands_FILE,
        //         orientation);
    ...
    default:
        ...
        play_ascii(internal_state, &sgftree, &gameinfo, infilename, untilstring);
        break;
    }
